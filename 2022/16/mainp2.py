...
nodes_to_work_on = [Node('AA', 0, 26-1, remaing_nodes, 0)]
print(nodes_to_work_on)

# first minute done manually
...
while len(nodes_to_work_on) != 0:
    valve = nodes_to_work_on.pop()
    if len(nodes_to_work_on) != 0:
        valve2 = nodes_to_work_on.pop()
    else:
        valve2 = None
    #print(f"{valve.remaining_nodes=}")
    returned = get_distance([valve, valve2], set(valves.keys()))
    #print(f"{valve=}")
    #print(f"{returned=}")
    for i in returned:
        valve.add_child(i)
...
print("Answer for sample is 1707")
